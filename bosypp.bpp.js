...
continue;
}}if (((obj.value)) === "extends") {{code+="extends ";
continue;
...
j<usings.length;
++j){if(modules[usings[j]]){console.log(modules[usings[j]],"'"+obj.value+"'",modules[usings[j]].indexOf(obj.value)!==-1);
if(modules[usings[j]].indexOf(obj.value)!==-1){code+=usings[j]+".";
break;
}}}code+=obj.value;
}return code;
}
